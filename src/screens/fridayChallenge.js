...
export default function fridayChallengea() {
  ...
  useEffect(() => {
    ...
    if (readCount == 20) {
      ...
      Alert.alert("You have finished reading Surat Al-Kahf", "My Alert Msg");
    }
  }, [readCount]);

  return (
    <View style={{ flex: 1, backgroundColor: "#0D013A" }}>
      <Header name={"Friday Challenge"} showBack={true} />
      {verses.length != 0 ? (
        <View
          style={{
            flex: 1,
        ...
        >
          ...
          <Text> You have read {readCount} of 110 Ayahs</Text>
        </View>
      ) : null}
    </View>
  );
